...
def get_all_ies():
    ...
    try:
        try:
            ...
            # Some extractor submodule is missing; attempt a safe import or fall back to empty list
            try:
                import importlib

                mod = importlib.import_module("yt_dlp.extractor.extractors")
                _ALL_CLASSES = getattr(mod, "_ALL_CLASSES", [])
            except ModuleNotFoundError:
                _ALL_CLASSES = []
    finally:
