...
# Lazy-import wrapper for yt_dlp.extractor to avoid importing extractors at module import time
_yt_dlp_extractor = None


def _get_yt_dlp_extractor():
    global _yt_dlp_extractor
    if _yt_dlp_extractor is None:
        try:
            import yt_dlp.extractor as _extractor
        except Exception:
            _extractor = None
        _yt_dlp_extractor = _extractor
    return _yt_dlp_extractor
...
def gettestcases(include_onlymatching=False):
    extractor = _get_yt_dlp_extractor()
    if extractor is None:
        return
    for ie in extractor.gen_extractors():
        try:
            yield from ie.get_testcases(include_onlymatching)
        except Exception:
            # Skip extractors that fail to provide testcases (e.g., missing optional deps)
            continue
...
def getwebpagetestcases():
    ...
    for ie in extractor.gen_extractors():
        try:
            ...
            # Skip extractors that cannot be loaded or don't provide webpage testcases
            continue
