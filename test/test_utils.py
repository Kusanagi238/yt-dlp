...
class _LazyProxy:
    ...
    def _load(self):
        if self._cached is None:
            ...
            self._cached = getattr(mod, self._attr)
        return self._cached
...
# Lazily import a few compat helpers
_compat_names = [
...
for _name in _compat_names:
    globals()[_name] = _LazyProxy("yt_dlp.compat", _name)

# Lazily import many utils symbols to avoid importing extractors at test-collection time
_utils_names = [
    "Config",
    "DateRange",
    "ExtractorError",
...
for _name in _utils_names:
    globals()[_name] = _LazyProxy("yt_dlp.utils", _name)

# Lazily import networking helpers
_networking_names = [
    "HTTPHeaderDict",
    "escape_rfc3986",
    "normalize_url",
...
for _name in _networking_names:
    globals()[_name] = _LazyProxy("yt_dlp.utils.networking", _name)
...
class TestUtil(unittest.TestCase):
    ...
    def test_update_url_query(self):
        ...
        self.assertEqual(
            parse_qs(update_url_query("http://example.com/path", {"bitrate": 5020.43})),
    ...
    def test_remove_dot_segments(self):
        ...
        self.assertEqual(
            remove_dot_segments("/abc/./.././d/././e/.././f/./../../ghi"), "/ghi"
        )
        self.assertEqual(remove_dot_segments("/"), "/")
    ...
    def test_js_to_json_edgecases(self):
        ...
        on = js_to_json(R'"\^\$\#"')
        self.assertEqual(
            json.loads(on), R"^$#", msg="Unnecessary escapes should be stripped"
        )

        on = js_to_json('\'"\\""\'')
        self.assertEqual(
            json.loads(on), '"""', msg="Unnecessary quote escape should be escaped"
        )
    ...
    def test_parse_resolution(self):
        ...
        self.assertEqual(
            parse_resolution("pre_1920x1080_post"), {"width": 1920, "height": 1080}
        )
    ...
    def test_detect_exe_version(self):
        self.assertEqual(
            detect_exe_version("""ffmpeg version 1.2.1
built on May 27 2013 08:37:26 with gcc 4.7 (Debian 4.7.3-4)
configuration: --prefix=/usr --extra-"""),
            "1.2.1",
        )
        self.assertEqual(
            detect_exe_version("""ffmpeg version N-63176-g1fb4685
built on May 15 2014 22:09:06 with gcc 4.8.2 (GCC)"""),
            "N-63176-g1fb4685",
        )
        self.assertEqual(
            detect_exe_version("""X server found. dri2 connection failed!
...
ffmpeg version 2.4.4 Copyright (c) 2000-2014 the FFmpeg ..."""),
    ...
    def test_render_table(self):
        ...
        self.assertEqual(
            render_table(
                ["a", "empty", "bcd"], [[123, "", 4], [9999, "", 51]], hide_empty=True
            ),
            "a    bcd\n" "123  4\n" "9999 51",
        )
        ...
        self.assertEqual(
            render_table(["a", "bcd"], [[123, 4], [9999, 51]], delim="-", extra_gap=2),
            "a      bcd\n" "----------\n" "123    4\n" "9999   51",
        )
...
"""
    ...
    def test_get_element_html_by_attribute(self):
        ...
        self.assertEqual(
            get_element_html_by_attribute("class", "foo bar", html), html.strip()
        )
        ...
        self.assertEqual(
            get_element_html_by_attribute("itemprop", "author", html), html.strip()
        )
    ...
    def test_get_elements_html_by_class(self):
        ...
        self.assertEqual(
            get_elements_html_by_class("foo", html), self.GET_ELEMENTS_BY_CLASS_RES
        )
    ...
    def test_get_elements_text_and_html_by_attribute(self):
        ...
        self.assertEqual(
            list(get_elements_text_and_html_by_attribute("class", "foo", html)), []
        )
        self.assertEqual(
            list(get_elements_text_and_html_by_attribute("class", "no-such-foo", html)),
            [],
        )
    ...
    """
    GET_ELEMENT_BY_TAG_RES_OUTERDIV_HTML = GET_ELEMENT_BY_TAG_TEST_STRING.strip()[
        32:276
    ]
    GET_ELEMENT_BY_TAG_RES_OUTERDIV_TEXT = GET_ELEMENT_BY_TAG_RES_OUTERDIV_HTML[5:-6]
    GET_ELEMENT_BY_TAG_RES_INNERSPAN_HTML = GET_ELEMENT_BY_TAG_TEST_STRING.strip()[
        78:119
    ]
    ...
    def test_get_element_text_and_html_by_tag(self):
        ...
        self.assertRaises(
            compat_HTMLParseError, get_element_text_and_html_by_tag, "article", html
        )
                    ...
                    for test_mode in MODES:
                        ...
                        try:
                            ...
                            if not testing_write:  # FIXME
                                ...
                                continue
                            self.assertTrue(
                                testing_write, f"{test_mode} is blocked by {lock_mode}"
                            )
                        else:
                            self.assertFalse(
                                testing_write,
    ...
    def test_determine_file_encoding(self):
        ...
        self.assertEqual(
            determine_file_encoding(b"# coding: someencodinghere-12345\n--verbose"),
    ...
    def test_get_compatible_ext(self):
        ...
        self.assertEqual(
            get_compatible_ext(
                vcodecs=["h264"], acodecs=["mp4a"], vexts=["mov"], aexts=["m4a"]
            ),
            "mp4",
        )
        self.assertEqual(
    ...
    def test_try_call(self):
        ...
        self.assertEqual(
            try_call(lambda: 1), 1, msg="int fn with no expected_type should give int"
        )
        self.assertEqual(
        ...
        self.assertEqual(
            try_call(total, kwargs={"a": 0, "b": 1, "c": 0}, expected_type=int),
            1,
    ...
    def test_traverse_obj(self):
        ...
        self.assertCountEqual(
            traverse_obj(_TEST_DATA, ...),
            (item for item in _TEST_DATA.values() if item not in (None, {})),
            msg="`...` should give all non discarded values",
        )
        self.assertCountEqual(
            traverse_obj(_TEST_DATA, ("urls", 0, ...)),
            _TEST_DATA["urls"][0].values(),
            msg="`...` selection for dicts should select all values",
        )
        self.assertEqual(
            traverse_obj(_TEST_DATA, (..., ..., "url")),
            ["https://www.example.com/0", "https://www.example.com/1"],
            msg="nested `...` queries should work",
        )
        self.assertCountEqual(
            traverse_obj(_TEST_DATA, (..., ..., "index")),
            range(4),
            msg="`...` query result should be flattened",
        )
        self.assertEqual(
            traverse_obj(iter(range(4)), ...),
            list(range(4)),
        ...
        self.assertEqual(
            traverse_obj(_TEST_DATA, lambda x, y: x == "urls" and isinstance(y, list)),
            [_TEST_DATA["urls"]],
        ...
        self.assertEqual(
            traverse_obj(_TEST_DATA, {dict}),
            _TEST_DATA,
            msg="A single set should be wrapped into a path",
        )
        self.assertEqual(
            traverse_obj(_TEST_DATA, (..., {str.upper})),
            ["STR"],
            msg="Transformation function should not raise",
        )
        self.assertEqual(
            traverse_obj(_TEST_DATA, (..., {str_or_none})),
            [
                item
        ...
        self.assertEqual(
            traverse_obj(_TEST_DATA, ("fail", {lambda _: "const"})),
            "const",
        ...
        self.assertEqual(
            traverse_obj(_TEST_DATA, (..., "fail"), 100),
            100,
            msg="alternatives should track their own branching return",
        )
        self.assertEqual(
            traverse_obj(_TEST_DATA, ("dict", ...), ("data", ...)),
        ...
        self.assertCountEqual(
            traverse_obj(_TEST_DATA, ("urls", ((1, ("fail", "url")), (0, "url")))),
            ["https://www.example.com/0", "https://www.example.com/1"],
        ...
        self.assertEqual(
            traverse_obj(_TEST_DATA, {0: ("urls", ((1, ("fail", "url")), (0, "url")))}),
        ...
        self.assertEqual(
            traverse_obj(_TEST_DATA, {0: "fail"}, default=...),
            {0: ...},
        ...
        self.assertEqual(
            traverse_obj(_TEST_DATA, {0: "dict"}, default=...),
            {0: ...},
        ...
        self.assertEqual(
            traverse_obj(None, {0: "fail"}), {}, msg="default to dict if pruned"
        )
        self.assertEqual(
            traverse_obj(None, {0: "fail"}, default=...),
            {0: ...},
            msg="default to dict if pruned and default is given",
        )
        self.assertEqual(
            traverse_obj(_TEST_DATA, {0: {0: "fail"}}, default=...),
            {0: {0: ...}},
        ...
        self.assertEqual(
            traverse_obj(_DEFAULT_DATA, (..., "fail"), default=None),
            None,
        ...
        self.assertEqual(
            traverse_obj(_EXPECTED_TYPE_DATA, "int", expected_type=lambda x: str(x)),
            "0",
            msg="transform type using type function",
        )
        self.assertEqual(
            traverse_obj(_EXPECTED_TYPE_DATA, "str", expected_type=lambda _: 1 / 0),
            None,
            msg="wrap expected_type fuction in try_call",
        )
        self.assertEqual(
            traverse_obj(_EXPECTED_TYPE_DATA, ..., expected_type=str),
            ["str"],
        ...
        self.assertEqual(
            traverse_obj(_TEST_DATA, ({0: 1.2}, 0, {int_or_none}), expected_type=int),
            1,
            msg="expected_type should not filter non final dict values",
        )
        self.assertEqual(
            traverse_obj(_TEST_DATA, {0: {0: 100, 1: "str"}}, expected_type=int),
            {0: {0: 100}},
        ...
        self.assertEqual(
            traverse_obj(_GET_ALL_DATA, ("key", ...), get_all=False),
            0,
            msg="if not `get_all`, return only first matching value",
        )
        self.assertEqual(
            traverse_obj(_GET_ALL_DATA, ..., get_all=False),
            [0, 1, 2],
        ...
        self.assertEqual(
            traverse_obj(_TRAVERSE_STRING_DATA, ("str", 0), traverse_string=True),
            "s",
        ...
        self.assertEqual(
            traverse_obj(_TRAVERSE_STRING_DATA, ("str", ...), traverse_string=True),
            "str",
            msg="`...` should result in string (same value) if `traverse_string`",
        )
        self.assertEqual(
            traverse_obj(
                _TRAVERSE_STRING_DATA, ("str", slice(0, None, 2)), traverse_string=True
            ),
            "sr",
        ...
        self.assertEqual(
            traverse_obj(_TRAVERSE_STRING_DATA, ("str", (0, 2)), traverse_string=True),
            ["s", "r"],
        ...
        self.assertEqual(
            traverse_obj(etree, 0), etree[0], msg="int key should return the nth child"
        )
        self.assertEqual(
        ...
        self.assertEqual(
            traverse_obj(etree, ("country", 0, ..., "text()", {int_or_none})),
            [1, 2008, 141100],
            msg="special transformations should act on current element",
        )
