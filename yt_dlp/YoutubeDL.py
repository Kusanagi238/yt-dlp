...
# Extractor imports are optional and may fail in some environments.
# Delay and guard imports to avoid import-time failures for missing optional extractors.
def _ensure_extractor_imports():
    global gen_extractor_classes, get_info_extractor, UnsupportedURLIE, PhantomJSwrapper
    try:
        from .extractor import gen_extractor_classes, get_info_extractor  # noqa: F401
        from .extractor.common import UnsupportedURLIE  # noqa: F401
        from .extractor.openload import PhantomJSwrapper  # noqa: F401
    except ModuleNotFoundError:
        # Provide safe fallbacks to allow module import to succeed.
        gen_extractor_classes = lambda: []
        get_info_extractor = lambda *a, **k: None

        class UnsupportedURLIE(Exception):
...
# Initialize fallbacks eagerly; real imports will be attempted when needed.
...
class YoutubeDL:
    ...
    def __init__(self, params=None, auto_init=True):
        ...
        if auto_init:
            try:
                ...
                try:
                    self.write_debug(
                        f"Skipping loading default info extractors due to missing module: {e}"
                    )
                except Exception:
                    pass
    ...
    def add_default_info_extractors(self):
        ...
        try:
            ...
            # Missing optional extractor modules should not break import/usage
            try:
                self.write_debug(f"Failed to load extractor modules: {e}")
            except Exception:
                pass
            ies = []
        all_ies = {ie.IE_NAME.lower(): ie for ie in ies}
    ...
    def print_debug_header(self):
        ...
        try:
            ...
            # Fall back to safe defaults if optional extractor modules are missing
            _LAZY_LOADER = False
            plugin_ies = {}
            plugin_ie_overrides = {}
